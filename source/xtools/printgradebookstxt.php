...
//Start drawing page...
  ...
  $outputtxt = "$nl $nl $nl";
  $outputtxt .= "Gradebook Report for $firstname $lastname   (ID $sid)$nl $nl";
...
// Process any Misc 3 tags...
  ...
  $outputtxt .= "Course Title: " . $grade_array[coursename][1] . "$nl";
  $outputtxt .= "Instructor: " . $hlink . "$nl";
  $outputtxt .= "Phone: " . $grade_array[phone][1] . "$nl";
  $outputtxt .= "Last Updated: " . $grade_array[modified][1] . "$nl $nl";
  $outputtxt .= "Current Grade: " . $grade_array[grade][1]."$nl";
  $outputtxt .= "Current Percent: " . $grade_array[percent][1] . "$nl $nl";
...
//If we encounter a #!C in $ealr[0], we have a commentary type record -- implode the array
 ...
 $outputtxt .= "$nl $nl $nl";
 $outputtxt .=  "Disclaimer:   The scores contained in the table above may be weighted.  $nl";
 $outputtxt .=  "It is not usually possible to simply add the number of points earned and divide $nl";
 $outputtxt .=  "by the total number of points when scores are weighted.  The scores presented in $nl";
 $outputtxt .=  "this table are for information only.$nl $nl";
 $outputtxt .=  "An asterisk (*) indicates that this score is missing from the teacher's grade book.$nl";
 $outputtxt .=  "Most likely this indicates that the assignment was never turned into the teacher.$nl";
 $outputtxt .=  "However, in rare occasions it may also indicate that this assignment was not$nl";
 $outputtxt .=  "intended to be graded.  If you have questions, please contact the school.$nl";
 $output .=  "<p>";
 $output .=  "<br>";
 $output .=  "</td></tr></table>";
...
// echo "$output <hr>";
  ...
  //Write the output to a file...
   $outfilename =    trim($StorageDir) . "gb." . trim($lastname) . "~" . trim($sid) . "~" . trim($cc) . ".txt";
   echo $outfilename . "<hr>";
   $fwpointer = fopen($outfilename,"w");
   fputs($fwpointer,$outputtxt);
   fclose($fwpointer);
